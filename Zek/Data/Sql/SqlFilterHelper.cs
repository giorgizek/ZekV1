...
namespace Zek.Data
{
    ...
    /// <summary>
    /// ფილტრის დამხმარე კლასი
    /// შენიშვნა: Reference-ში ჭირდება Zek.Common
    /// </summary>
    public class SqlFilterHelper
    {
        ...
        {
            return (value is string && ((string)value).Trim().Length == 0) || value.IsNullOrDefault();
        }
        ...
        {
            if (whereOperator.IsNullOrWhiteSpace()) return string.Empty;
        ...
        public static string GetWhereClause(string fieldName, WhereOperator whereOperator, bool doNotFilterIfEmpty, DateTime? value, DateTimePrecision precission)
        {
            ...
            value = value.RoundDown(precission);
            switch (whereOperator)
            {
                case WhereOperator.Equals:
                    output = fieldName + " >= " + value.ToSQLValue() + " AND " + fieldName + " < " + value.AddOneUnit(precission).ToSQLValue();
                    break;

                case WhereOperator.NotEquals:
                    output = fieldName + " < " + value.ToSQLValue() + " OR " + fieldName + " >= " + value.AddOneUnit(precission).ToSQLValue();
                    break;

                case WhereOperator.GreaterThan:
                    output = fieldName + " >= " + value.AddOneUnit(precission).ToSQLValue();
                    break;
        ...
        public static string GetWhereClause(string fieldName, WhereOperator whereOperator, bool doNotFilterIfEmpty, DateTime? value1, DateTime? value2, DateTimePrecision precission)
        {
            ...
            if (doNotFilterIfEmpty && (value1.IsNullOrDefault() || value2.IsNullOrDefault()))
                return string.Empty;
            ...
            var output = string.Empty;
            value1 = value1.RoundDown(precission);
            switch (whereOperator)
            {
                case WhereOperator.Between:
                    output = fieldName + " >= " + value1.ToSQLValue() + " AND " + fieldName + " < " + value2.RoundDown(precission).AddOneUnit(precission).ToSQLValue();
                    break;

                case WhereOperator.NotBetween:
                    output = fieldName + " < " + value1.ToSQLValue() + " OR " + fieldName + " >= " + value2.RoundDown(precission).AddOneUnit(precission).ToSQLValue();
                    break;
            }
