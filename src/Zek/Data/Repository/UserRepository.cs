...
namespace Zek.Data.Repository
{
        ...
        where TEntity : User
    {
        ...
        Task<TEntity> FindByIdAndRowIdAsync(int? id, Guid? rowId);
        ...
        where TEntity : User
    {
        ...
        public async Task<TEntity> FindByIdAndRowIdAsync(int? id, Guid? rowId)
        {
            if (id.GetValueOrDefault() <= 0 || rowId == null)
                return null;
            return await Where(u => u.Id == id && u.RowId == rowId).SingleOrDefaultAsync();
        }
